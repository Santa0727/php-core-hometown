        ...
        switch( $action ) {
            ...
            case 'fav-stores':
                if( ( $pagination = have_favorites() ) && $pagination['results'] > 0 ) {
                    echo '<div class="row">';
                    foreach( favorites( array( 'orderby' => 'date desc' ) ) as $item ) {
                        echo couponscms_store_item( $item, 4 );
                    }
                    echo '</div>';
            ...
            case 'fav-coupons':
                if( ( $pagination = have_favorite_items() ) && $pagination['results'] > 0 ) {
                    echo '<div class="row">';
                    foreach( favorite_items( array( 'orderby' => 'date desc' ) ) as $item ) {
                        echo couponscms_coupon_item( $item, 4 );
                    }
                    echo '</div>';
                    echo couponscms_theme_pagination( $pagination );
            ...
            case 'fav-products':
                if( ( $pagination = have_favorite_products() ) && $pagination['results'] > 0 ) {
                    echo '<div class="row">';
                    foreach( favorite_products( array( 'orderby' => 'date asc' ) ) as $item ) {
                        echo couponscms_product_item( $item, 4 );
                    }
                    echo '</div>';
                    echo couponscms_theme_pagination( $pagination );
            ...
            case 'saved-stores':
                if( ( $pagination = have_saved_stores() ) && $pagination['results'] > 0 ) {
                    echo '<div class="row">';
                    foreach( saved_stores( array( 'orderby' => 'added_date desc' ) ) as $item ) {
                        echo couponscms_store_item( $item, 4 );
                    }
                    echo '</div>';
                    echo couponscms_theme_pagination( $pagination );
            ...
            case 'saved-coupons':
                if( ( $pagination = have_saved_items() ) && $pagination['results'] > 0 ) {
                    echo '<div class="row">';
                    foreach( saved_items( array( 'orderby' => 'added_date desc' ) ) as $item ) {
                        echo couponscms_coupon_item( $item, 4 );
                    }
                    echo '</div>';
                    echo couponscms_theme_pagination( $pagination );
            ...
            case 'saved-products':
                if( ( $pagination = have_saved_products() ) && $pagination['results'] > 0 ) {
                    echo '<div class="row">';
                    foreach( saved_products( array( 'orderby' => 'added_date desc' ) ) as $item ) {
                        echo couponscms_product_item( $item, 4 );
                    }
                    echo '</div>';
                    echo couponscms_theme_pagination( $pagination );
            ...
            case 'rewards':
                if( ( $pagination = have_rewards() ) && $pagination['results'] > 0 ) {
                    echo '<div class="row">';
                    foreach( rewards( array( 'orderby' => 'name desc' ) ) as $item ) {
            ...
            case 'reward-reqs':
                if( ( $pagination = have_reward_reqs() ) && $pagination['results'] > 0 ) {
                    echo '<div class="row">';
                    foreach( reward_reqs( array( 'orderby' => 'date desc' ) ) as $item ) {
            ...
            case 'my-stores':
                if( ( $pagination = have_stores( array( 'show' => 'all' ) ) ) && $pagination['results'] > 0 ) {
                    ...
                    foreach( stores( array( 'orderby' => 'date desc', 'show' => 'all' ) ) as $item ) {
                        echo couponscms_store_item( $item, 4, true );
                    }
                    echo '</div>';
                    echo couponscms_theme_pagination( $pagination );
            ...
            case 'my-coupons':
                if( ( $pagination = have_coupons( array( 'show' => 'all' ) ) ) && $pagination['results'] > 0 ) {
                    ...
                    foreach( coupons( array( 'orderby' => 'date desc', 'show' => 'all' ) ) as $item ) {
                        echo couponscms_coupon_item( $item, 4, true );
                    }
                    echo '</div>';
                    echo couponscms_theme_pagination( $pagination );
            ...
            case 'my-products':
                if( ( $pagination = have_products( array( 'show' => 'all' ) ) ) && $pagination['results'] > 0 ) {
                    ...
                    foreach( products( array( 'orderby' => 'date desc', 'show' => 'all' ) ) as $item ) {
                        echo couponscms_product_item( $item, 4, true );
                    }
                    echo '</div>';
                    echo couponscms_theme_pagination( $pagination );
            ...
            case 'purchase':
                if( ( $pagination = have_payment_plans() ) && $pagination['results'] > 0 ) {
                    echo '<div class="row">';
                    foreach( payment_plans( array( 'orderby' => 'date desc' ) ) as $item ) {
                        echo couponscms_plans_item( $item );
                    }
                    echo '</div>';
                    echo couponscms_theme_pagination( $pagination );
                } else echo '<div class="alert">' . t( 'theme_no_saved_coupons',  'No plans at this moment. Please check again later.' ) . '</div>';
            break;
