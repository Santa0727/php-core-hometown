...
function couponscms_search_form( $extra_class = '' ) {
    echo '<div class="general-search-container search-container' . ( !empty( $extra_class ) ? ' ' . $extra_class : '' ) . '">
        <div class="container">
            <div class="row sc-title">
                <div class="col-md-7">';
                    echo '<h2>' . 'Search for <span style="background-color: white;">big discounts</span> in your favorite stores' . '</h2>
                </div>
            </div>
            <div class="row sc-form">
                <form action="' . site_url() . '" autocomplete="off" method="GET" class="homepage-search-form">
                    <div class="col-md-7">
...
/* Search Form Custom */
function my_search_form( $extra_class = '' ) {
    echo '<div class="my-search-container search-container' . ( !empty( $extra_class ) ? ' ' . $extra_class : '' ) . '">
        <div class="container">
            <div class="row sc-title">
                <div class="col-md-7">';
            ...
            <div class="row sc-form">
                <form action="' . site_url() . '" autocomplete="off" method="GET">
                    ...
                    <div class="col-md-2 sc-select">
                        <div class="category-select" id="city-button-select">
                            <a href="#"><span>' . t( 'search_by_city', 'SEARCH BY CITY' ) . '</span> <i class="fa fa-map-marker"></i></a>';
                    echo'</div>
                    </div>
                    <div class="col-md-3 sc-select">
                        <div class="category-select" id="category-button-select">
                            <a href="#"><span>' . t( 'search_by_category', 'SEARCH BY CATEGORY' ) . '</span> <i class="fa fa-tags"></i></a>';
                    echo '</div>
                    </div>';
                    echo '<div class="col-md-12 my-item-box category-buttons sc-select">';
                        foreach( all_grouped_categories() as $category ) {
                            echo '<div class="category-select my-item-button"><a href="'. $category['info']->link.'" data-attr="'.$category['info']->ID.'">'.$category['info']->name.'</a></div>';
                        }
                    echo '</div>';
                    echo '<div class="col-md-12 my-item-box city-buttons sc-select">';
                        foreach(all_city_id_names() as $city) {
                            echo '<div class="category-select my-item-button"><a href="'.site_url().'?s=city&city='.$city->ID.'">'.$city->name.'</a></div>';
                        }
                echo '</div>
                </form>
            </div>
...
function couponscms_home_items() {
    ...
    $all_items = items_custom( array( 'show' => 'all', 'orderby' => 'date', 'max' => 10000 ) );
    $section_count = 0;
    $markup = '';
    $is_exist = false;
    ...
    foreach($all_items as $row) {
        ...
        if($count++ >= 8) break;
        $is_exist = true;
        $markup .= "<div class='coupon-no-".($count%3)."'>";
    ...
    function cmp($first, $second) {
        $res = strcmp($first->date, $second->date);
        if($res == 0) return $res;
        return $res > 0 ? -1 : 1;
    }
    usort($all_items, "cmp");
