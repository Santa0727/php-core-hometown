...
if( empty( $info->source ) ) {
    ...
    if( \query\locations::store_locations( array( 'store' => $info->storeID ) ) ) {
        ...
        foreach( \query\locations::while_store_locations( array( 'max' => 0, 'store' => $info->storeID ) ) as $loc ) {
            echo "<a target='blank' href='".map_url($loc->lat, $loc->lng)."'><li><span>".$loc->address."</span></li>";
            echo "<li><span>".$loc->city.", ".strtoupper(substr($loc->state, 0, 2))." ".$loc->zip."</span></li></a>";
        }
        echo '</ul>';
    } else {
        echo '-';
    ...
    if( !empty( $info->store_phone_no ) ) {
        echo '<li><span>' . $info->store_phone_no . '</span></li>';
    }
    echo '</li>
    </ul>
    </div>
    ...
    <ul>';
    echo '<li><span>' . t( 'form_description', "Description" ) . ':</span> ' . ( !empty( $info->description ) ? $info->description : '-' ) . '</li>';
    echo '<li><span>Disclaimer:</span> ' . ( !empty( $info->disclaimer ) ? $info->disclaimer : '-' ) . '</li>';
    // if( !$info->is_started ) {
        // echo '<li><span>' . t( 'form_start_date', "Start Date" ) . ':</span> ' . date( 'm/d/Y', strtotime( $info->start_date ) ) .'</li>';
    // }
    echo '<li><span> Expires :</span> ' . date( 'm/d/Y', strtotime( $info->expiration_date ) ) .'</li>';
    echo '</ul>
        <li style="margin-top: 50%;"><span>Coupon found at </span><span style="font-weight: bolder;font-size: 1.25rem;">HometownOC.coms<span></li>
    </div></div>';
}
else {
...
<a href="' . site_url() . '">&larr; ' . t( 'back', "Back" ) . '</a> <br />
