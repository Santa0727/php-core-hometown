...
public static function have_items( $category = array(), $place = '', $special = array() ) {
    ...
    if( !empty( $categories['store_name'] ) ) {
        $search = implode( '.*', explode( ' ', trim( $categories['store_name'] ) ) );
        $where[] = 's.name REGEXP "' . \site\utils::dbp( $search ) . '"';
    }
    /* */
