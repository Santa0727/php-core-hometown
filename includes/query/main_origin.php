...
public static function store_info( $store_id = 0, $special = array() ) {
    ...
    $stmt->prepare( "SELECT s.id, s.feedID, s.user, s.category, s.popular, s.physical, s.name, s.link, s.description, s.disclaimer, s.tags, s.image, s.hours, s.phoneno, s.sellonline, s.visible, s.views, (SELECT COUNT(*) FROM " . DB_TABLE_PREFIX . "coupons WHERE store = s.id AND visible > 0), s.url_title, s.meta_title, s.meta_keywords, s.meta_desc, s.lastupdate_by, (SELECT name FROM " . DB_TABLE_PREFIX . "users WHERE id = s.lastupdate_by), s.lastupdate, s.extra, s.date, (SELECT COUNT(*) FROM " . DB_TABLE_PREFIX . "reviews WHERE store = s.id AND valid > 0), (SELECT AVG(stars) FROM " . DB_TABLE_PREFIX . "reviews WHERE store = s.id AND valid > 0), (SELECT COUNT(*) FROM " . DB_TABLE_PREFIX . "products WHERE store = s.id AND visible > 0), u.name FROM " . DB_TABLE_PREFIX . "stores s LEFT JOIN " . DB_TABLE_PREFIX . "users u ON (u.id = s.user) WHERE s.id = ? OR s.url_title = ?" );
    $stmt->bind_param( "is", $store_id, $store_id );
    $stmt->execute();
    $stmt->bind_result( $id, $feed_id, $user, $cat, $popular, $physical, $name, $link, $description, $disclaimer, $tags, $image, $hours, $phone, $sellonline, $visible, $views, $coupons, $url_title, $meta_title, $meta_keywords, $meta_desc, $lastupdate_by, $lastupdate_by_name, $last_update, $extra, $date, $reviews, $stars, $products, $user_name);
    $stmt->fetch();
    $stmt->close();
