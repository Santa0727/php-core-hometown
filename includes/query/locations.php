...
/* Get Only City Id and Names */

public static function get_city_id_names( $special = array() ) {
    ...
    $stmt = $db->stmt_init();
    $stmt->prepare( "SELECT id, user, name, state, lat, lng FROM " . DB_TABLE_PREFIX . "cities" );
    $stmt->execute();
    $stmt->bind_result( $id, $user, $name, $state, $lat, $lng );

    $data = array();
    while( $stmt->fetch() ) {
        $data[] = (object) array( 'ID' => $id, 'user' => $user, 'name' => esc_html( $name ), 'state' => $state, 'lat' => $lat, 'lng' => $lng );
    }
