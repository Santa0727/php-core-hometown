...
public static function have_city_store( $category = array(), $special = array() ) {

    global $db;

    $city_id = $category['city'];
    $active = false;
    ...
    $stmt = $db->stmt_init();
    $stmt->prepare("SELECT COUNT(DISTINCT(store_locations.store)) FROM store_locations JOIN stores on store_locations.store = stores.id where store_locations.cityID = ".$city_id.($active ? " AND stores.visible = 1" : ""));
    $stmt->execute();
    $stmt->bind_result( $count );
    $stmt->fetch();
    $stmt->close();
...
public static function city_stores( $category = array(), $place = '', $special = array() ) {
    ...
    if( !empty( $categories['city'] ) ) {
        $where[] = 'sl.cityID = "' . \site\utils::dbp( $categories['city'] ) . '"';
    }
