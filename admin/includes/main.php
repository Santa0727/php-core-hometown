...
class main
{
    ...
    public function default_links()
    {
        ...
        if ($GLOBALS['me']->is_admin) {
            $nav['contest_winners']['name'] = '<a href="?route=contest_winners.php">Contest Winners</a>';
            $nav['contest_winners']['class'] = 'contest_winners';
            $nav['contest_winners']['position'] = 18;
        }
    ...
    <input type="text" name="link" value="' . (!empty($info->original_url) ? $info->original_url : 'http://') . '"' . (empty($info->original_url) ? ' style="display:none;"' : '') . ' />
        ...
        $fields['disclaimer']['position'] = 11;
        $fields['disclaimer']['markup'] = '<div class="row"><span>' . 'Disclaimer' . ':</span><div><textarea name="disclaimer">' . (isset($info->disclaimer) ? $info->disclaimer : '') . '</textarea></div></div>';

        // Tags
        $fields['tags']['position'] = 12;
    ...
    <div>';
        ...
        $fields['image']['position'] = 13;
        $fields['image']['markup'] = $image;

        // Cashback
        $fields['cashback']['position'] = 14;
        ...
        // Last verification
        $fields['last_verification']['position'] = 16;
        ...
        // Votes average
        $fields['votes_average']['position'] = 18;
    ...
    <ul' . (empty($info->hours) ? ' style="display: none;"' : '') . '>';
        ...
        if (\query\main::get_option('hour_format') == 12) {
            ...
            $hours = array(
                '01:00 AM', '01:15 AM', '01:30 AM', '01:45 AM', '02:00 AM', '02:15 AM', '02:30 AM', '02:45 AM', '03:00 AM', '03:15 AM', '03:30 AM ', '03:45 AM ', '04:00 AM ', '04:15 AM', '04:30 AM', '04:45 AM',
        ...
        $fields['hours']['position'] = $position_count++;
        $fields['hours']['markup'] = $hours_markup;

        // Check if sells online
        $fields['sell_online']['position'] = $position_count++;
        ...
        // Store's URL
        $fields['url']['position'] = $position_count++;
        ...
        // Description
        $fields['description']['position'] = $position_count++;
        $fields['description']['markup'] = '<div class="row"><span>' . t('form_description', "Description") . ':</span><div><textarea name="description">' . (isset($info->description) ? $info->description : '') . '</textarea></div></div>';

        // Tags
        $fields['tags']['position'] = $position_count++;
        $fields['tags']['markup'] = '<div class="row"><span>' . t('form_tags', "Tags") . ':</span><div><input type="text" name="tags" value="' . (isset($info->tags) ? $info->tags : '') . '" /></div></div>';
    ...
    <div>';
        ...
        $fields['image']['position'] = $position_count++;
        $fields['image']['markup'] = $image;

        // Phone no
        $fields['phone']['position'] = $position_count++;
        ...
        // Add to
        $fields['add_to']['position'] = $position_count++;
    ...
    <span class="fright date">' . date('m.d,Y ' . (\query\main::get_option('hour_format') == 12 ? 'g:i A' : 'G:i'), strtotime($item->date)) . '</span></h2>
    ...
    <span class="fright date">' . date('m.d,Y ' . (\query\main::get_option('hour_format') == 12 ? 'g:i A' : 'G:i'), strtotime($item->date)) . '</span></h2></div>
