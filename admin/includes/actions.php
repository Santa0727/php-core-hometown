...
public static function add_item( $opt = array() ) {
    ...
    $stmt = $db->stmt_init();
    $stmt->prepare( "INSERT INTO " . DB_TABLE_PREFIX . "coupons (feedID, user, store, category, popular, exclusive, printable, show_in_store, available_online, title, link, description, disclaimer, tags, image, code, source, claim_limit, visible, start, expiration, cashback, meta_title, meta_keywords, meta_desc, votes, votes_percent, verified, last_verif, lastupdate_by, lastupdate, extra, date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), ?, NOW())" );
    ...
    $stmt->bind_param( "iiiiiiiiissssssssiississsidisis", $feedID, $GLOBALS['me']->ID, $opt['store'], $opt['category'], $opt['popular'], $opt['exclusive'], $opt['printable'], $opt['show_in_store'], $opt['available_online'], $opt['name'], $opt['link'], $opt['description'], $opt['disclaimer'], $opt['tags'], $image, $opt['code'], $source, $opt['claim_limit'], $opt['publish'], $opt['start'], $opt['end'], $opt['cashback'], $opt['meta_title'], $opt['meta_keywords'], $opt['meta_desc'], $opt['votes'], $opt['votes_average'], $opt['verified'], $opt['last_verif'], $GLOBALS['me']->ID, $extra );
    $execute = $stmt->execute();
    $insert_id = $stmt->insert_id;
    $stmt->close();
...
public static function edit_item( $id, $opt = array() ) {
    ...
    $stmt->bind_param( "iiiiiiissssssssiississsidisisi", $opt['store'], $opt['category'], $opt['popular'], $opt['exclusive'], $opt['printable'], $opt['show_in_store'], $opt['available_online'], $opt['name'], $opt['link'], $opt['description'], $opt['disclaimer'], $opt['tags'], $image, $opt['code'], $source, $opt['claim_limit'], $opt['publish'], $opt['start'], $opt['end'], $opt['cashback'], $opt['meta_title'], $opt['meta_keywords'], $opt['meta_desc'], $opt['votes'], $opt['votes_average'], $opt['verified'], $opt['last_verif'], $GLOBALS['me']->ID, $extra, $id );
    $execute = $stmt->execute();
    $stmt->close();
