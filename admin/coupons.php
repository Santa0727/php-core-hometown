...
echo '<input type="search" style="margin-right:5px;" name="by_store" value="' . (isset( $_GET['by_store'] ) ? esc_html( $_GET['by_store'] ) : '') . '" placeholder="Search Business Name" />';
...
$p = \query\main::have_items( ( $options = value_with_filter( 'admin_view_items_args', array( 'per_page' => 10, 'store' => ( isset( $_GET['store'] ) ? $_GET['store'] : '' ), 'user' => ( isset( $_GET['user'] ) ? $_GET['user'] : '' ), 'categories' => ( isset( $_GET['category'] ) ? $_GET['category'] : '' ), 'show' => ( isset( $_GET['view'] ) ? $_GET['view'] : 'all' ), 'search' => ( isset( $_GET['search'] ) ? urldecode( $_GET['search'] ) : '' ), 'store_name' => ( isset( $_GET['by_store'] ) ? urldecode( $_GET['by_store'] ) : '' ) ) ) ) );
